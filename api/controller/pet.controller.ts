...
export class PetController {
  ...
  async getById(id: number | string): Promise<operations["getPetById"]["responses"]["200"]["schema"]> { // correct example
    const response = await this.instance.get(`${id}`);
    return response.data;
  }
  async findByStatus(status: string | string[]): Promise<operations["findPetsByStatus"]["responses"]["200"]["schema"]> {
  ...
  async addNew(pet: Omit<definitions["Pet"], "id">): Promise<definitions["Pet"]>{
